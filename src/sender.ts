...
export async function send(tg: Telegram, channelId: string, plan: RenderPlan): Promise<void> {
  if (plan.mode === 'inline') {
    await tg.api.sendMessage({ chat_id: channelId, text: plan.text, parse_mode: 'HTML' })
    return
  }
  ...
  await tg.api.sendMessage({ chat_id: channelId, text: plan.followup, parse_mode: 'HTML' })
}
